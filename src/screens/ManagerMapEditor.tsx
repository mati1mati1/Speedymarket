...
const ItemTypes = {
  ...
  const ManagerMapEditor: React.FC = () => {
    ...
    useEffect(() => {
      // Load the map from the DB when the component mounts
      loadMapFromDB();
    }, []);
    ...
    const saveMapToDB = async () => {
      ...
      const response = await fetch('http://localhost:7071/api/saveMap', {
        ...
        body: JSON.stringify({ supermarketId: 'supermarketId', mapData })
      });
    ...
    const loadMapFromDB = async () => {
      const response = await fetch('http://localhost:7071/api/loadMap', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ supermarketId: 'supermarketId' })
      });
  
      if (response.ok) {
        const mapData = await response.json();
        setSections(mapData.sections || []);
        setEntrance(mapData.entrance || null);
      } else {
